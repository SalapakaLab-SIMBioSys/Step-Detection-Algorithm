...
%%
function EstY = GetStepsHMM1(Y, verbose)
    ...
    % Analysis parameters
    SimpleFB = 0;  % 1: Use the simple but fast FFT-based calculation. No prior.
                   % 0: use the full HMM.
    ...
    if(verbose == 1)
        ...
        if ntx>0  % We plot the X trace, detecting steps assuming it is noiseless.
            subplot(2,1,2);
            RestorationPlot(EstY,X1,nu,M.YQuantum,0,'b+');
            title('Restored timecourse (dots) and X');
            subplot(2,1,1);
        end;
        RestorationPlot(Y1,EstY,nu,M.YQuantum);
